...
<body>
				...
				<table class="am-table">
					<thead>
						<tr>
							<!-- <th><input type="checkbox" name="selid"
								onclick="checkAll('videoId',this)"></th> -->
							<th>视频名称</th>
							<th>视频简介</th>
							<th>视频格式</th>
							<th>创建时间</th>
					...
					<tbody>
						<c:forEach items="${movieList}" var="m">
							<tr>
								<%-- <td><input type="checkbox" name="videoId"
									value="${m.videoId}" /></td> --%>
								<td>${m.videoname}</td>
								<td>${m.brief}</td>
								...
								<td><fmt:formatDate value="${m.updateTime}"
										pattern="yyyy-MM-dd HH:mm:ss" /></td>
								<td>
									<input type="button" onclick="toView(this)" value="查看" data-id="${m.videoId}"/>
								</td>
							</tr>
						</c:forEach>
					</tbody>
	...
	function toView(othis){
		var id = $(othis).data('id');
		window.location.href = "${scp}/backend/movie/movieInfo?videoId="+id;
	}
	</script>
